...
// MonoBehavior 를 상속받은 컴포넌트들은 생성자를 통해서 생성하면 안됨.
public class Movement : MonoBehaviour
{
    /// <summary>
    /// 스크립트 인스턴스가 처음 로드될 때 호출
    /// 해당 스크립트인스턴스를 컴포넌트로 포함하는 게임 오브젝트가 활성화 되어있어야 호출
    /// (게임오브젝트가 비활성화 된 채로실행되면 해당 게임 오브젝트는 인스턴스화 되지 않고, 따라서 스크립트 인스턴스도 인스턴스화 되지않는다)
    /// 스크립트인스턴스를 비활성화하겠다고 해 놓아도
    /// 게임 오브젝트가 인스턴스화 될때 해당 스크립트 인스턴스도 로드되기 떄문에 Awake는 호출된다.
    ...
    /// 이 스크립트를 컴포넌트로 가지는 GameObject 가 Trigger를 가지고 있던지, other 의 GameObject 가 Trigger를 가지고있던지
    /// 둘중 하나라도 RigidBody 를 가지고 있어야 물리적인 연산이 적용되므로 단순히 Trigger-Trigger 또는 Trigger-Collider 끼리의 충돌이 아니라
    /// 둘중에 하나라도 RigidBody를 가지고 있고 , 둘중에 하나라도 Trigger를 가지고 있어야한다.
    /// </summary>
    ...
    {
        Debug.Log($"[Movement] : {collision.gameObject.name} 가 충돌 됨");
    }
